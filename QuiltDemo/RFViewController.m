...
@interface RFViewController () <UICollectionViewDelegate, UICollectionViewDataSource_Draggable> {
  BOOL isAnimating;
}
...
{
  NSUInteger idx = fromIndexPath.item;
  NSNumber *index = [[self numbers] objectAtIndex:idx];
  [[self numbers] removeObjectAtIndex:idx];
  [[self numbers] insertObject:index atIndex:idx];
  
  NSNumber *indexW = [[self numberWidths] objectAtIndex:idx];
  [[self numberWidths] removeObjectAtIndex:idx];
  [[self numberWidths] insertObject:indexW atIndex:idx];
  
  NSNumber *indexH = [[self numberHeights] objectAtIndex:idx];
  [[self numberHeights] removeObjectAtIndex:idx];
  [[self numberHeights] insertObject:indexH atIndex:idx];
}
